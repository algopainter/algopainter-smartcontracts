...
contract.only('AlgoPainterRewardsSystem', (accounts) => {
  ...
  it('Each bidback stake must compute all bidback reward percentages', async () => {
    ...
    await algopToken.approve(rewardsSystemManager.address, stakeAmount, { from: accounts[1] });
    await rewardsSystemManager.stakeBidback(auctionId, stakeAmount, { from: accounts[1] });
    ...
    await algopToken.approve(rewardsSystemManager.address, stakeAmount2, { from: accounts[2] });
    await rewardsSystemManager.stakeBidback(auctionId, stakeAmount2, { from: accounts[2] });

    const stakeAmount3 = web3.utils.toWei('300', 'ether');
    await algopToken.approve(rewardsSystemManager.address, stakeAmount3, { from: accounts[3] });
    await rewardsSystemManager.stakeBidback(auctionId, stakeAmount3, { from: accounts[3] });
    await rewardsSystemManager.unstakeBidback(auctionId, stakeAmount3, { from: accounts[3] });
  ...
  it('Each bidback unstake must compute all bidback reward percentages', async () => {
    ...
    await rewardsSystemManager.unstakeBidback(auctionId, unstakeAmount, { from: accounts[1] });
  ...
  it('Users can\'t stake or unstake bidback on an ended auction', async () => {
    ...
    try {
      ...
      await rewardsSystemManager.stakeBidback(0, web3.utils.toWei('100', 'ether'), { from: accounts[1] });
      throw {};
    } catch (error) {
    ...
    try {
      await rewardsSystemManager.unstakeBidback(0, web3.utils.toWei('100', 'ether'), { from: accounts[2] });
      throw {};
    } catch (error) {
      expect(error.reason).to.be.equal('AlgoPainterRewardsSystem: AUCTION_ENDED');
    }
  ...
  it('Claim bidback', async () => {
    ...
    await rewardsSystemManager.claimBidback(0, { from: accounts[2] });
